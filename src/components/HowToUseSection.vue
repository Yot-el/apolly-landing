<script setup>
	...
	onMounted(() => {
		controlTimeline
			.to('.video__control-container', {
				'--first-circle-size': '90px',
				'--second-circle-size': '90px',
				duration: 0.5
			})
			.to(
				'.video__control-container',
				{ '--second-circle-size': '110px', '--circles-opacity': '0' },
				'>-=0.4'
			)
			.to(
				'.video__control',
				{ width: 70, height: 70, opacity: 0, duration: 0.4 },
				'>-=0.4'
			)
			.reverse()
	})
...
<template>
	<SectionLayout class="section--has-background">
		<template #title> How to use the app perfectly </template>
		...
		<template #content>
			...
			>
				...
				>
					<source
						src="@/assets/videos/how-to-use.mp4"
						type="video/mp4"
					/>
				</video>
			</div>
